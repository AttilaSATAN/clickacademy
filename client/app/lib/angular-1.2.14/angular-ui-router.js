...
(function (window, angular, undefined) {
    ...
    function $Resolve($q, $injector) {
        ...
        this.study = function (invocables) {
            if (!isObject(invocables)) throw new Error(
            ...
            function visit(value, key) {
                ...
                if (isString(value)) {
                    ...
                    forEach(params, function (param) {
                        if (param !== key && invocables.hasOwnProperty(
                            param)) visit(invocables[param], param);
                    });
            ...
            return function (locals, parent, self) {
                ...
                function invoke(key, invocable, params) {
                    ...
                    forEach(params, function (dep) {
                        if (promises.hasOwnProperty(dep) && !locals.hasOwnProperty(
                            dep)) {
                            waitParams++;
                    ...
                    function proceed() {
                        ...
                        try {
                            invocation.resolve($injector.invoke(invocable,
                                self, values));
    ...
    function $TemplateFactory($http, $templateCache, $injector) {
        ...
        this.fromConfig = function (config, params, locals) {
            return (isDefined(config.template) ? this.fromString(config.template,
                    params) : isDefined(config.templateUrl) ? this.fromUrl(
                    config.templateUrl, params) : isDefined(config.templateProvider) ?
    ...
    function UrlMatcher(pattern) {
        ...
        function addParameter(id) {
            if (!/^\w+(-+\w+)*$/.test(id)) throw new Error(
                "Invalid parameter name '" + id + "' in pattern '" +
                pattern + "'");
            if (names[id]) throw new Error("Duplicate parameter name '" +
                id + "' in pattern '" + pattern + "'");
            names[id] = true;
            params.push(id);
        }
    ...
    UrlMatcher.prototype.exec = function (path, searchParams) {
        ...
        var params = this.params,
            ...
            values = {}, i;
        if (nPath !== m.length - 1) throw new Error(
            "Unbalanced capture group in route '" + this.source + "'");
        for (i = 0; i < nPath; i++) values[params[i]] = m[i + 1];
        for ( /**/ ; i < nTotal; i++) values[params[i]] = searchParams[
            params[i]];
        return values;
    };
    /**
    ...
    function $UrlMatcherFactory() {
        ...
        this.isMatcher = function (o) {
            return isObject(o) && isFunction(o.exec) && isFunction(o.format) &&
                isFunction(o.concat);
        };
        this.$get = function () {
            return this;
    ...
    function $UrlRouterProvider($urlMatcherFactory) {
        ...
        this.rule = function (rule) {
            if (!isFunction(rule)) throw new Error(
        ...
        this.otherwise = function (rule) {
            ...
            } else if (!isFunction(rule)) throw new Error(
                "'rule' must be a function");
            otherwise = rule;
            return this;
        };
        ...
        this.when = function (what, handler) {
            ...
            if (!handlerIsString && !isFunction(handler) && !isArray(
                handler)) throw new Error("invalid 'handler' in when()");
            var strategies = {
                matcher: function (what, handler) {
                    ...
                    return extend(function ($injector, $location) {
                        return handleIfMatch($injector, handler, what.exec(
                ...
                regex: function (what, handler) {
                    if (what.global || what.sticky) throw new Error(
            ...
            function ($location, $rootScope, $injector) {
                ...
                function update(evt) {
                    ...
                    function check(rule) {
                        ...
                        if (handled) {
                            if (isString(handled)) $location.replace()
                                .url(handled);
                            return true;
                        }
                        return false;
    ...
    angular.module('ui.router.router')
     ...
     */
    $StateProvider.$inject = ['$urlRouterProvider',
    ...
    function $StateProvider($urlRouterProvider, $urlMatcherFactory,
        ...
        var stateBuilder = {
            ...
            parent: function (state) {
                if (isDefined(state.parent) && state.parent) return findState(
                    state.parent);
            ...
            data: function (state) {
                if (state.parent && state.parent.data) {
                    state.data = state.self.data = extend({}, state.parent.data,
                        state.data);
                }
                return state.data;
            },
            ...
            url: function (state) {
                ...
                if (isString(url)) {
                    ...
                    return (state.parent.navigable || root)
                        .url.concat(url);
                }
                if ($urlMatcherFactory.isMatcher(url) || url == null) {
                    return url;
                }
                throw new Error("Invalid url '" + url + "' in state '" +
                    state + "'");
            },
            ...
            params: function (state) {
                if (!state.params) {
                    return state.url ? state.url.parameters() : state.parent
                        .params;
                }
                if (!isArray(state.params)) throw new Error(
                    "Invalid params in state '" + state + "'");
                if (state.url) throw new Error(
            ...
            views: function (state) {
                ...
                forEach(isDefined(state.views) ? state.views : {
                    ...
                    if (name.indexOf('@') < 0) name += '@' + state.parent
                        .name;
                    views[name] = view;
                });
                return views;
        ...
        function isRelative(stateName) {
            return stateName.indexOf(".") === 0 || stateName.indexOf("^") ===
                0;
        }
        ...
        function findState(stateOrName, base) {
            ...
            if (path) {
                ...
                for (; i < pathLength; i++) {
                    ...
                    if (rel[i] === "^") {
                        if (!current.parent) throw new Error("Path '" +
                            name + "' not valid for state '" + base.name +
                            "'");
                ...
                rel = rel.slice(i)
                    .join(".");
                name = current.name + (current.name && rel ? "." : "") +
                    rel;
            }
            var state = states[name];
            if (state && (isStr || (!isStr && (state === stateOrName ||
        ...
        function registerState(state) {
            ...
            var name = state.name;
            if (!isString(name) || name.indexOf('@') >= 0) throw new Error(
                "State must have a valid name");
            if (states.hasOwnProperty(name)) throw new Error("State '" +
                name + "'' is already defined");
            // Get parent name
            var parentName = (name.indexOf('.') !== -1) ? name.substring(0,
                name.lastIndexOf('.')) : (isString(state.parent)) ? state.parent :
                '';
            ...
            for (var key in stateBuilder) {
                if (isFunction(stateBuilder[key])) state[key] =
                    stateBuilder[key](state, stateBuilder.$delegates[key]);
            }
            states[name] = state;
            // Register the state in the global state list and with $urlRouter if necessary.
            if (!state[abstractKey] && state.url) {
                $urlRouterProvider.when(state.url, ['$match',
                    '$stateParams',
        ...
        this.$get = $get;
        $get.$inject = ['$rootScope', '$q', '$view', '$injector',
            '$resolve', '$stateParams', '$location', '$urlRouter'
        ];

        function $get($rootScope, $q, $view, $injector, $resolve,
            $stateParams, $location, $urlRouter) {
            var TransitionSuperseded = $q.reject(new Error(
                'transition superseded'));
            var TransitionPrevented = $q.reject(new Error(
                'transition prevented'));
            var TransitionAborted = $q.reject(new Error(
                'transition aborted'));
             ...
             * @param {object} options If Object is passed, object is an options hash.
             */
            $state.transitionTo = function transitionTo(to, toParams,
                options) {
                toParams = toParams || {};
                options = extend({
                    location: true,
                ...
                if (!isDefined(toState)) {
                    ...
                    evt = $rootScope.$broadcast('$stateNotFound', redirect,
                    ...
                    if (evt.retry) {
                        ...
                        var retryTransition = $state.transition = $q.when(
                            evt.retry);
                        retryTransition.then(function () {
                            if (retryTransition !== $state.transition)
                                return TransitionSuperseded;
                            redirect.options.$retry = true;
                            return $state.transitionTo(redirect.to,
                                redirect.toParams, redirect.options);
                        }, function () {
                            return TransitionAborted;
                        });
                        syncUrl();
                    ...
                    if (!isDefined(toState)) {
                        if (options.relative) throw new Error(
                            "Could not resolve '" + to + "' from state '" +
                            options.relative + "'");
                        throw new Error("No such state '" + to + "'");
                    }
                }
                if (toState[abstractKey]) throw new Error(
                    "Cannot transition to abstract state '" + to + "'");
                if (options.inherit) toParams = inheritParams($stateParams,
                ...
                var keep, state, locals = root.locals,
                    toLocals = [];
                for (keep = 0, state = toPath[keep]; state && state ===
                    fromPath[keep] && equalForKeys(toParams, fromParams,
                        state.ownParams) && !options.reload; keep++, state =
                    toPath[keep]) {
                ...
                if (options.notify) {
                    evt = $rootScope.$broadcast('$stateChangeStart', to.self,
                        toParams, from.self, fromParams);
                    if (evt.defaultPrevented) {
                        syncUrl();
                ...
                for (var l = keep; l < toPath.length; l++, state = toPath[l]) {
                    ...
                    resolved = resolveState(state, toParams, state === to,
                        resolved, locals);
                }
                ...
                var transition = $state.transition = resolved.then(function () {
                    ...
                    for (l = fromPath.length - 1; l >= keep; l--) {
                        ...
                        if (exiting.self.onExit) {
                            $injector.invoke(exiting.self.onExit,
                                exiting.self, exiting.locals.globals);
                        }
                        exiting.locals = null;
                    }
                    ...
                    for (l = keep; l < toPath.length; l++) {
                        ...
                        if (entering.self.onEnter) {
                            $injector.invoke(entering.self.onEnter,
                                entering.self, entering.locals.globals);
                        }
                    }
                    ...
                    if (options.location && toNav) {
                        $location.url(toNav.url.format(toNav.locals.globals
                            .$stateParams));
                    ...
                    if (options.notify) {
                        $rootScope.$broadcast('$stateChangeSuccess', to
                            .self, toParams, from.self, fromParams);
                    }
                    ...
                    if ($state.transition !== transition) return TransitionSuperseded;
                    $state.transition = null;
                    $rootScope.$broadcast('$stateChangeError', to.self,
                        toParams, from.self, fromParams, error);
                    syncUrl();
            ...
            $state.is = function is(stateOrName, params) {
                ...
                return isDefined(params) && params !== null ? angular.equals(
                    $stateParams, params) : true;
            };
            /**
             * @ngdoc function
            ...
            $state.includes = function includes(stateOrName, params) {
                ...
                angular.forEach(params, function (value, key) {
                    if (!isDefined($stateParams[key]) || $stateParams[
                        key] !== value) {
                        validParams = false;
                    }
                });
            ...
            $state.href = function href(stateOrName, params, options) {
                ...
                params = inheritParams($stateParams, params || {}, $state.$current,
                    state);
                var nav = (state && options.lossy) ? state.navigable :
                    state;
                var url = (nav && nav.url) ? nav.url.format(normalize(state
                    .params, params || {})) : null;
                ...
                if (options.absolute && url) {
                    url = $location.protocol() + '://' + $location.host() +
                        ...
                        '' : ':' + $location.port()) + (!$locationProvider.html5Mode() &&
                        url ? '/' : '') + url;
                }
                return url;
            };
            ...
            function resolveState(state, params, paramsAreFiltered,
                inherited, dst) {
                // Make a restricted $stateParams with only the parameters that apply to this state if
                // necessary. In addition to being available to the controller and onEnter/onExit callbacks,
                // we also need $stateParams to be available for any $injector calls we make during the
                // dependency resolution process.
                var $stateParams = (paramsAreFiltered) ? params :
                ...
                forEach(state.views, function (view, name) {
                    var injectables = (view.resolve && view.resolve !==
                    ...
                    promises.push($resolve.resolve(injectables, locals,
                            dst.resolve, state)
        ...
        function $get($rootScope, $templateFactory) {
            return {
                ...
                load: function load(name, options) {
                    ...
                    if (options.view) {
                        result = $templateFactory.fromConfig(options.view,
                            options.params, options.locals);
                    }
     ...
     */
    $ViewDirective.$inject = ['$state', '$compile', '$controller',
        '$injector', '$uiViewScroll', '$document'
    ];

    function $ViewDirective($state, $compile, $controller, $injector,
        $uiViewScroll, $document) {
        function getService() {
            return ($injector.has) ? function (service) {
                return $injector.has(service) ? $injector.get(service) :
                    null;
            } : function (service) {
                try {
        ...
        var directive = {
            ...
            compile: function (element, attrs) {
                var initial = element.html(),
                    isDefault = true,
                    anchor = angular.element($document[0].createComment(
                        ' ui-view-anchor ')),
                    ...
                    function updateView(shouldAnimate) {
                        ...
                        if (!locals) {
                            ...
                            renderer(shouldAnimate)
                                .enter(currentEl, parentEl, anchor);
                        ...
                        cleanupLastView();
                        currentEl = element.clone();
                        currentEl.html(locals.$template ? locals.$template :
                            initial);
                        renderer(true)
                            .enter(currentEl, parentEl, anchor);
                        ...
                        if (locals.$$controller) {
                            ...
                            var controller = $controller(locals.$$controller,
                                locals);
                            currentEl.children()
    ...
    angular.module('ui.router.state')
        .directive('uiView', $ViewDirective);

    function parseStateRef(ref) {
        var parsed = ref.replace(/\n/g, " ")
            .match(/^([^(]+?)\s*(\((.*)\))?$/);
        if (!parsed || parsed.length !== 4) throw new Error(
        ...
        return {
            ...
            link: function (scope, element, attrs, uiSrefActive) {
                ...
                element.bind("click", function (e) {
                    ...
                    if ((button === 0 || button == 1) && !e.ctrlKey && !
                        e.metaKey && !e.shiftKey && !element.attr(
                            'target')) {
     ...
     * </pre>
     */
    $StateActiveDirective.$inject = ['$state', '$stateParams',
        '$interpolate'
    ];
        ...
        return {
            ...
            controller: ['$scope', '$element', '$attrs',
                function ($scope, $element, $attrs) {
                    ...
                    activeClass = $interpolate($attrs.uiSrefActive || '',
                        false)($scope);
                    ...
                    function update() {
                        if (
                            ($state.$current.self === state &&
                                matchesParams()) /* Exact match */ || (
                                $attrs.uiNested !== undefined && $state.includes(
                                    state.name)) /* Match nested state */ ) {
    ...
    function $RouteProvider($stateProvider, $urlRouterProvider) {
            ...
            if (route.redirectTo != null) {
                ...
                if (isString(redirect)) {
                    ...
                    handler = function (params, $location) {
                        return redirect(params, $location.path(), $location
                            .search());
                    };
                } else {
        ...
        function $get($state, $rootScope, $routeParams) {
            ...
            $rootScope.$on('$stateChangeStart', function (ev, to, toParams,
                from, fromParams) {
                $rootScope.$broadcast('$routeChangeStart', stateAsRoute(
                    to), stateAsRoute(from));
            });
            $rootScope.$on('$stateChangeSuccess', function (ev, to,
                toParams, from, fromParams) {
                $route.current = stateAsRoute(to);
                $rootScope.$broadcast('$routeChangeSuccess',
                    stateAsRoute(to), stateAsRoute(from));
                copy(toParams, $route.params);
            });
            $rootScope.$on('$stateChangeError', function (ev, to, toParams,
                from, fromParams, error) {
                $rootScope.$broadcast('$routeChangeError', stateAsRoute(
                    to), stateAsRoute(from), error);
            });
            return $route;
        }
    }
